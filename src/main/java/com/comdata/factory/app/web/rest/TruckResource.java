...
public class TruckResource {
	 ...
	 *         new truck, or with status 400 (Bad Request) if the truck has
	 *         already an ID
	...
	public ResponseEntity<Truck> createTruck(@Valid @RequestBody TruckDTO dto) throws URISyntaxException {
		...
		if (dto.getId() != null) {
			return ResponseEntity.badRequest().headers(
					HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new truck cannot already have an ID"))
					.body(null);
		}
	 ...
	 *         truck, or with status 400 (Bad Request) if the truck is not
	 *         valid, or with status 500 (Internal Server Error) if the truck
	 *         couldn't be updated
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	...
	public ResponseEntity<TruckDTO> updateTruck(@Valid @RequestBody TruckDTO truck) throws URISyntaxException {
		...
		if (truck.getId() == null) {
			return ResponseEntity.badRequest().headers(
					HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "You can only update an existing truck."))
					.body(null);
		}
		Truck result = truckService.save(truck.convertToTruckEntity());
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, truck.getId().toString()))
	 ...
	 * @return the ResponseEntity with status 200 (OK) and the list of trucks in
	 *         body
	 */
	@GetMapping("/trucks")
	@Timed
	public ResponseEntity<List<TruckDTO>> getAllTrucks(@ApiParam Pageable pageable) {
		log.debug("REST request to get a page of Trucks");
		Page<Truck> page = truckService.findAll(pageable);
		List<Truck> allTrucks = page.getContent();
	 ...
	 * @return the ResponseEntity with status 200 (OK) and with body the truck,
